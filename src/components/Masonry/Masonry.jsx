...
export default function Masonry({
  ...
  const handleMouseEnter = (e, item) => {
    ...
    if (colorShiftOnHover) {
      const overlay = e.currentTarget.querySelector('.m-color-overlay');
  ...
  return (
    <div ref={containerRef} className="m-list">
      {grid.map(item => (
        <div
          key={item.id}
          data-key={item.id}
          data-selector={`[data-key="${item.id}"]`}
          className="m-item-wrapper"
          onMouseEnter={e => handleMouseEnter(e, item)}
          onMouseLeave={handleMouseLeave}
        >
          <div className="m-item-img" style={{ backgroundImage: `url(${item.img})` }}>
            {colorShiftOnHover && <div className="m-color-overlay" />}
            {item.label && <span className="m-item-label">{item.label}</span>}
          </div>
        </div>
      ))}
    </div>
